...
/* Main entry point to the bubble chart. This function is returned by the parent closure. It prepares the raisseData for visualization and adds an svg element to the provided selector and starts the visualization creation process. selector is expected to be a DOM element or CSS selector that points to the parent element of the bubble chart. Inside this element, the code will add the SVG continer for the visualization. rawData is expected to be an array of data objects as provided by a d3 loading function like d3.csv. */
...
// -----------------------------------------------------------------*/
  ...
  function splitBubblesintoGriffweitealter() {
    showGriffweitealter();
    hideYear();
    hideSex();
    hideAgecat();
    ...
    force.on('tick', function (e) {
      bubbles.each(moveToGriffweitealter(e.alpha))
        .attr('cx', function (d) { return d.x; })
        .attr('cy', function (d) { return d.y; });
    });
  ...
  function hideGriffweitealter() {
    svg.selectAll('.griffweitealter').remove();
  }
